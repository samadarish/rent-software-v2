...
export async function fetchTenantDocuments({ tenantId = "" } = {}) {
    ...
    try {
        ...
        console.error("fetchTenantDocuments error", err);
        showToast("Failed to load tenant documents", "error");
        return { ok: false, docs: [] };
    }
}

export async function uploadTenantDocument(payload, options = {}) {
    const url = ensureAppScriptUrl({
        promptForConfig: true,
        onMissing: () => showToast("Configure Apps Script URL to upload documents", "warning"),
    });
    if (!url) return { ok: false };

    const onProgress = options && typeof options.onProgress === "function" ? options.onProgress : null;
    const onAbort = options && typeof options.onAbort === "function" ? options.onAbort : null;
    const uploadId = options && options.uploadId ? String(options.uploadId) : `doc-${Date.now()}`;
    const tauriInvoke = window.__TAURI__?.core?.invoke;
    const tauriEvents = window.__TAURI__?.event;
    ...
    if (typeof tauriInvoke === "function") {
        ...
        try {
            const result = await tauriInvoke("upload_tenant_document", {
                url,
                payload,
                uploadId,
            });
            ...
            if (typeof unlisten === "function") unlisten();
            console.error("uploadTenantDocument error", err);
    ...
    try {
        const body = JSON.stringify({ action: "uploadTenantDocument", payload });
        const result = await new Promise((resolve) => {
            const xhr = new XMLHttpRequest();
            xhr.open("POST", url, true);
            xhr.setRequestHeader("Content-Type", "text/plain");
